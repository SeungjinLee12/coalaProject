...
const QNAList = () => {
  ...
  useEffect(() => {
    axios.get(`${serverUrl}/lecture/QNAAllList/`).then((res) => {
      setQNAList(res.data);
  ...
  const handleClick = (questionNo, userName, inserttime, questionTitle) => {
    navigate(`/lecture/QNA/?questionNo=${questionNo}`, {
      state: {
        userName,
        inserttime,
        questionNo,
  ...
  return (
    <div style={{ marginTop: "20px" }}>
      <div style={{ display: "flex" }}>
        <h1 style={{ marginLeft: "20px" }}>Q&A</h1>
        ...
        <tbody>
          {reversedQuestions.map((QNAList, index) => (
            <tr key={index}>
              ...
              >
                {QNAList.questionTitle.length > 20
                  ? QNAList.questionTitle.substring(0, 20) + "..."
                  : QNAList.questionTitle}
              </td>
              <td style={{ border: "1px solid #ddd", padding: "8px" }}>
